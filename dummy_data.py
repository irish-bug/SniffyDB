...
def main():
    ...
    print("1. tables reset and packets from dhcp.pcap added to the packet table")
    init_db.main()
    os.system("./pcap2db.sh ../pcaps/dhcp.pcap")
    print("2. dummy tag (Bob) created on pin=2 packet's SRC (192.168.0.1)")
    connection = init_db.connect_database()
    add_tag(connection, 'Bob', 'SRC')
    add_tagged(connection, 1, 'dhcp.pcap', 2)
    connection.close()
    msg = "3. add packets from pcap2.json (which is just a slightly modified version of dhcp.pcap)" \
